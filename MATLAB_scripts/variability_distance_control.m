...
for j = 1:24
    ...
    ht_n = [];
    pull_n = [];
    if isempty(consistent_windows) 
        disp("Error for"+mousename)
    ...
    for i = 1:length(consistent_windows)
        ...
        if length([blocks{consistent_windows(i):consistent_windows(i)+window_size,4}])~= 21
            ht_n(i,:) = [blocks{consistent_windows(i):consistent_windows(i)+window_size,4},zeros(1,21-length([blocks{consistent_windows(i):consistent_windows(i)+window_size,4}]))];
        else
...
selectedColumns_WT = [1,3,17,22,23,24]; %WT
selectedData_WT = hts_neg{:, selectedColumns_WT};
selectedColumns_Q = [5,7,8,14,19,20]; %Q175 11 and 12 removed
selectedData_Q = hts_neg{:, selectedColumns_Q};
...
xlabel('Trial Number');
ylabel('Fold Change in HoldTime');
title('DTW Distance');